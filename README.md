...
Для второй реализации я уже реализовал изменение/удаление/перемещение элемента списка с использованием рекурсивных
функций. Этот вариант хорошо работает, но на каждую такую операцию требуется перебирать массив рекурсивно (пусть и не
целиком, а только до момента, пока не найдётся нужный элемент).<br>
Придумал, как избавиться от рекурсивных функций и ускорить выполнение нужных действий.<br>
Решение: Я сохраню адрес данного элемента и передам его в компонент item. Так же, как и в первой реализации, но с
поправкой на то, что данные хранятся в нескольких вложенных массивах.<br>
Значит индекс-указатель будет состоять из нескольких номеров, пример: [3, 2, 0] - указатель на элемент "Text 4-3-1". А
далее, при удалении или перемещении элемента вместо рекурсивного поиска, будет выполняться очень быстрый и простой поиск
нужного элемента по индексу.<br>
